...
- 调整计划结束日期时，从今天（未开始的计划从起始日）到目标日期按剩余时长重排：先合并同一视频的未完成切片，再复用计划算法按秒均分到学习日，同日同一视频的未完成部分只保留一段；跳过周末的计划只排工作日。已完成记录不变；重建后的切片数量可能变化，原任务 ID 优先分配给同一视频的新切片，新增切片使用新 ID，旧提前归位历史不得覆盖新排期。
